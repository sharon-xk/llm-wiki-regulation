...
CONCEPT_RULES = {
    ...
    "关联交易违规": [
        r"关联交易",
        r"未.*建立.*关联交易.*制度",
        r"关联方.*混同", r"与.*关联.*(?:混同|共用)",
        r"与.*关联机构.*共用",
        r"关联账户.*对倒", r"关联.*账户.*交易",
        r"人员.*办公.*混同", r"人员.*场地.*混同",
        r"办公场所.*混同", r"办公场地.*混同",
    ],
    "虚假登记备案": [
    ...
    "违规募集": [
        ...
        # OCR 截断：募集被截成"募"或"禁止募"
        r"禁止[幕募]", r"违规.*[幕募](?!集)",
        r"[幕募].*集.*行为.*管理",
        r"宣传.*材料.*误导", r"推介.*材料.*误导",
        r"夸大.*片面.*宣传", r"片面.*宣传.*业绩",
    ],
    "投资者适当性违规": [
        r"非合格投资者", r"不合格投资者",
        r"向.*合格投资者.*之外.*[幕募莫慕].*集",
        ...
        # 未履行程序类
        r"未履行.*特定对象.*确认", r"未履行.*特定对象",
        r"未履行.*投资者.*风险评估", r"未履行.*风险评估",
        r"未履行.*风险.*揭示", r"未履行.*合格投资者.*确认",
    ],
    "信息披露违规": [
        r"信息披露", r"信息.*披露",
    ...
    # ── 人员特有违规类型 ──
    "违反从业资格管理": [
        r"未取得.*从业资格", r"未取得.*基金从业",
        r"不具备.*从业资格", r"不具备.*基金从业",
        r"无.*从业资格.*从事", r"无.*基金从业.*从事",
        r"从业资格.*不符", r"基金从业资格.*不符",
        r"未.*取得.*从业资格.*从事",
        r"提供.*虚假.*从业",
        r"从业资格.*虚假", r"基金从业.*虚假",
        r"出租.*出借.*从业资格", r"出借.*从业资格",
        r"违规.*取得.*从业资格",
        r"资格.*被.*撤销.*继续",
    ],
    "违反专业化运营": [
        r"兼营.*(?:业务|与.*无关)", r"从事.*与.*私募.*无关",
        r"兼营.*非.*私募", r"兼营.*冲突.*业务",
        r"违反.*专业化.*经营", r"违反.*专业化.*运营",
        r"未.*遵守.*专业化.*经营",
        r"兼职.*(?:管理|担任).*职务", r"在外.*兼职",
        r"违反.*从业.*独立性", r"违反.*人员.*独立性",
        r"高级管理人员.*在.*非.*本.*机构.*兼职",
        r"高管.*在.*(?:其它|其他).*机构.*兼职",
        r"未建立.*从业人员.*管理制度",
        r"从业人员.*管理.*违规",
        r"违规.*兼职", r"员工.*违规.*兼职",
    ...
    "不配合自律管理": [
        r"拒绝.*配合.*(?:检查|调查|自律)", r"不.*配合.*(?:检查|调查)",
        r"拒绝.*提供.*(?:材料|资料|信息)", r"拒绝.*提供.*相关.*(?:材料|资料)",
        r"未.*按要求.*提供", r"未.*按.*规定.*提供.*材料",
        r"提供.*虚假.*材料.*(?:检查|调查|自律)",
        r"隐瞒.*(?:重要|关键|相关).*事实", r"隐瞒.*真实.*情况",
        r"提供.*虚假.*陈述", r"提供.*不真实.*信息",
        r"拒绝.*接受.*询问", r"拒绝.*签字",
        r"逃避.*(?:检查|监管|自律)",
        r"未.*配合.*协会.*(?:检查|调查)",
    ],
